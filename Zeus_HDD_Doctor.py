...
from PyQt5.QtWidgets import (
    ...
    QDialog # QDialog'u import ediyoruz
)
...
def get_disk_list():
    ...
    try:
        ...
        QMessageBox.critical(None, "Hata", f"lsblk komutu çalıştırılırken sorun oluştu: {e.stderr.decode('utf-8')}")
        return []
...
def get_smart_data(disk_path):
    ...
    for dev_type in device_types:
        try:
            ...
            error_message = f"smartctl '{dev_type}' tipiyle '{disk_path}' için çalıştırılamadı. Hata: {e.stderr.decode('utf-8').strip()}"
            attributes_output = None
            info_output = None
        except FileNotFoundError:
...
class AboutDialog(QDialog):
    ...
    def init_ui(self):
        ...
        title_label = QLabel("Zeus HDD Doctor")
        title_label.setFont(QFont("Arial", 16, QFont.Bold))
        title_label.setAlignment(Qt.AlignCenter)
        layout.addWidget(title_label)

        version_label = QLabel("Sürüm: 1.0.0") # Sürüm bilgisi
...
class ZeusHDDDoctor(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Zeus HDD Doctor")
    ...
    def init_ui(self):
        ...
        left_panel.addWidget(self.disk_list_widget)
        ...
        button_layout.addStretch(1) # Boşluk bırak
        ...
        right_panel.addLayout(button_layout) # Yatay layout'u ana layout'a ekle
    ...
    def show_about_dialog(self):
        ...
        about_dialog.exec_() # Modsal bir pencere olarak açar
    ...
    def display_disk_data(self, disk_path):
        ...
        if attributes_output and info_output:
            ...
            if smart_attributes:
                ...
                if health_score >= 85:
                    self.health_status_label.setStyleSheet("background-color: #57E389; padding: 10px; border-radius: 5px;")
                    self.notes_text.setStyleSheet("background-color: #e0ffe0; border: 1px solid #c0e0c0; padding: 5px; border-radius: 5px;")
                elif health_score >= 70:
                    self.health_status_label.setStyleSheet("background-color: #C6E357; padding: 10px; border-radius: 5px;")
                    self.notes_text.setStyleSheet("background-color: #f0ffe0; border: 1px solid #e0e0c0; padding: 5px; border-radius: 5px;")
                elif health_score >= 60:
                    self.health_status_label.setStyleSheet("background-color: #FFD43D; padding: 10px; border-radius: 5px;")
                ...
                self.health_status_label.setText("Sağlık: Bilgi Yok")
    ...
    if os.geteuid() != 0:
        ...
        try:
            ...
            QMessageBox.critical(None, "Yetkilendirme Hatası",
                                 ...
                                 f"Detay: {e.stderr.decode('utf-8').strip()}")
            sys.exit(1)
        except Exception as e:
